...
const solderMaskStateVariant = (state) => {
  switch (state) {
    case 'approved':
...
const SolderMaskDashboard = () => {
  ...
  const loadDashboardData = async (tokenValue) => {
    ...
    try {
      ...
      const readyForSolderMask = fetchedWorkOrders.filter((wo) => {
        if (!wo) return false;
        const stage = String(wo.stage || '').toLowerCase();
        const tinStripState = String(wo?.tinStrippingStatus?.state || '').toLowerCase();

        if (!stage) return false;
        if (stage === 'surface_finish') return false;

        // Require tin stripping approval before showing the order in solder mask
        const tinStrippingApproved = tinStripState === 'approved';

        if (stage === 'solder_mask') return tinStrippingApproved;
        if (stage === 'tin_strip') return tinStrippingApproved;

        return false;
      });

      setWorkOrders(readyForSolderMask);
    } catch (err) {
      toast({
  ...
  const renderContent = () => {
    switch (activeSection) {
      case 'work-orders':
        return (
          <WorkOrderBoard
            title="Solder Mask Station Work Queue"
            subtitle="Work orders requiring solder mask processing."
            ...
            columns={[
              ...
              {
                key: 'solderMaskFiles',
                label: 'Solder Mask Files',
                render: (wo) => {
                  const solderMaskFiles = (wo.camAttachments || []).filter(att =>
                    ['gerber', 'solder_mask_file', 'job_card'].includes(att.kind)
                  );
                  if (solderMaskFiles.length === 0) return <span className="text-muted-foreground">None</span>;
                  return (
                    <div className="flex flex-col gap-1">
                      {solderMaskFiles.map(file => (
                        <Button
                          key={file.filename}
                          variant="link"
                          size="sm"
              ...
              {
                key: 'solderMaskStatus',
                label: 'Solder mask state',
                render: (wo) => (
                  <div>
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${solderMaskStateVariant(
                        wo?.solderMaskStatus?.state
                      )}`}
                    >
                      {wo?.solderMaskStatus?.state || 'pending'}
                    </span>
                    <div className="text-xs text-muted-foreground">
                      Owner: {wo?.solderMaskStatus?.owner || '--'}
                    </div>
                  </div>
                )
              },
              ...
              },
              {
                ...
                render: (wo) => formatDateTime(wo?.solderMaskStatus?.releaseTarget)
              },
              {
                key: 'priority',
                label: 'Priority',
            ...
            emptyMessage="No work orders in your solder mask queue."
            ...
            renderActions={(wo) => (
              <>
                ...
                {hasPermission('solder_mask:release') && (
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={eventSubmitting}
                    onClick={() =>
                      handleTravelerAction('release', wo, { boardContext: 'solder_mask' })
                    }
                  >
                    Release
                  </Button>
                ...
                {hasPermission('qc:hold') && (
                  <Button
                    ...
                    disabled={eventSubmitting}
                    onClick={() =>
                      handleTravelerAction('hold', wo, { boardContext: 'solder_mask' })
                    }
                  >
                    Hold
                  </Button>
      ...
      case 'checklist':
        return (
          <ChecklistView
            ...
            token={token}
            station="solder_mask"
            checklistKey="solderMaskChecklist"
            statusKey="solderMaskStatus"
        ...
        return (
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-6">
                  {/* CAM Pipeline Section */}
                  <div>
                    <h3 className="text-lg font-semibold mb-4">CAM Process</h3>
                    <CamPipelinePanel
                      selectedWorkOrder={selectedWorkOrder}
                      highlightStage={selectedWorkOrder?.stage}
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {pcbPipelineStages.map((stage, index) => {
                        // For now, show all stages as pending since we don't have status tracking yet
                        const baseState = normalizeStageState(
                          getPcbStageStatus(selectedWorkOrder, stage.id)
                        );
                        const derivedState =
                          stage.id === 'solder_mask' && baseState === 'pending'
                            ? 'in_progress'
                            : baseState;
                        ...
                        return (
                          <div
                            ...
                            className={`border rounded-lg p-3 shadow-sm bg-white ${
                              stage.id === 'solder_mask' ? 'ring-2 ring-blue-500' : ''
                            }`}
                          >
  ...
  if (loading) {
    return (
      <DashboardLayout
        title="Solder Mask Station Dashboard"
        subtitle="Solder Mask Application & Curing"
        operator={operator}
        workCenter={operator?.workCenter || 'Solder Mask Station'}
        loading={loading}
        loadingMessage="Loading solder mask workspace..."
      />
    );
  }

  console.log('Rendering Solder Mask Station Dashboard with activeSection:', activeSection);
  return (
    <DashboardLayout
      title="Solder Mask Station Dashboard"
      subtitle="Solder Mask Application & Curing"
    ...
    >
      <div className="flex gap-6">
        <SolderMaskSidebar activeSection={activeSection} onNavigate={setActiveSection} />
        ...
        <div className="flex-1 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <SummaryTile
              ...
              description="Currently in solder mask"
            />
            <SummaryTile
              title="Due Today"
              value={workOrders.filter(wo => {
            ...
            <SummaryTile
              title="On Hold"
              value={workOrders.filter(wo => wo.status === 'on_hold' || wo.status === 'blocked').length || 0}
              status="warning"
...
export default SolderMaskDashboard;
