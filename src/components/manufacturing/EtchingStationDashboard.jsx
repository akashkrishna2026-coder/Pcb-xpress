...
const etchingStateVariant = (state) => {
  switch (state) {
    case 'approved':
...
const EtchingStationDashboard = () => {
  ...
  const renderContent = () => {
    switch (activeSection) {
      case 'work-orders':
        return (
          <WorkOrderBoard
            title="Etching Station Work Queue"
            subtitle="Work orders requiring etching processing."
            ...
            columns={[
              ...
              { key: 'product', label: 'Product' },
              {
                ...
                label: 'Etching Files',
                render: (wo) => {
                  const etchingFiles = (wo.camAttachments || []).filter(att =>
                    ['gerber', 'etching_file', 'job_card'].includes(att.kind)
                  );
                  if (etchingFiles.length === 0) return <span className="text-muted-foreground">None</span>;
                  return (
                    <div className="flex flex-col gap-1">
                      {etchingFiles.map(file => (
                        <Button
                          key={file.filename}
                          variant="link"
                          size="sm"
              ...
              },
              {
                ...
                render: (wo) => (
                  <div>
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${etchingStateVariant(
                        wo?.etchingStatus?.state
                      )}`}
                    >
                      {wo?.etchingStatus?.state || 'pending'}
                    </span>
                    <div className="text-xs text-muted-foreground">
                      Owner: {wo?.etchingStatus?.owner || '--'}
                    </div>
                  </div>
                )
              },
              ...
              },
              {
                ...
                render: (wo) => formatDateTime(wo?.etchingStatus?.releaseTarget)
              },
              {
                key: 'priority',
                label: 'Priority',
            ...
            emptyMessage="No work orders in your etching queue."
            ...
            renderActions={(wo) => (
              <>
                ...
                {hasPermission('etching:release') && (
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={processProcessEventSubmitting}
                    onClick={() =>
                      handleProcessAction('release', wo, { boardContext: 'etching' })
                    }
                  >
                    Release
                  </Button>
                ...
                {hasPermission('qc:hold') && (
                  <Button
                    ...
                    disabled={processProcessEventSubmitting}
                    onClick={() =>
                      handleProcessAction('hold', wo, { boardContext: 'etching' })
                    }
                  >
                    Hold
                  </Button>
      ...
      case 'checklist':
        return (
          <ChecklistView
            ...
            token={token}
            station="etching"
            checklistKey="etchingChecklist"
            statusKey="etchingStatus"
      ...
      case 'pcb-pipeline':
        return (
          <PcbPipelinePanel
            ...
            highlightStage="etching_station"
          />
        );
      ...
      case 'transfer':
        return (
          <TransferView
            workOrder={selectedWorkOrder}
            token={token}
            currentStage="etching"
            nextStage="tin_stripping"
            onTransfer={() => {
              loadDashboardData(token);
            }}
  ...
  if (loading) {
    return (
      <DashboardLayout
        title="Etching Station Dashboard"
        subtitle="Copper Etching & Patterning"
        operator={operator}
        workCenter={operator?.workCenter || 'Etching Station'}
        loading={loading}
        loadingMessage="Loading etching workspace..."
      />
    );
  }

  console.log('Rendering Etching Station Dashboard with activeSection:', activeSection);
  return (
    <DashboardLayout
      title="Etching Station Dashboard"
      subtitle="Copper Etching & Patterning"
    ...
    >
      <div className="flex gap-6">
        <EtchingSidebar activeSection={activeSection} onNavigate={setActiveSection} />
        ...
        <div className="flex-1 space-y-6">
          <EtchingTopbar
            ...
            loading={summaryLoading || boardsLoading || loading}
          />
...
export default EtchingStationDashboard;
