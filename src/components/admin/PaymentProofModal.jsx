...
const PaymentProofModal = ({ quote, onClose, onStatusUpdate }) => {
  ...
  const handleStatusUpdate = async (status) => {
    ...
    try {
      ...
      await api.adminUpdatePaymentProofStatus(
        token,
        quote._id || quote.id,
        status,
        rejectionReason,
      ...
      toast({ title: `Payment proof ${status}` });
      onStatusUpdate?.();
      onClose?.();
    } catch (err) {
        ...
        <div className="p-6 space-y-6">
          ...
          {proof?.status && proof.status !== 'submitted' && (
            <div className="space-y-2 text-sm text-muted-foreground">
              {proof.status === 'approved' && proof.approvedAt && (
                <p>Approved on {new Date(proof.approvedAt).toLocaleString()}</p>
              )}
