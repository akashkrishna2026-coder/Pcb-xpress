...
const AdminQuotesPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPage = async () => {
      ...
      if (t) {
        try {
          const res = await api.listAllQuotesAdmin(t, { limit, page, service: 'pcb' });
          setQuotes(res.quotes || []);
          setTotal(res.total || 0);
          setPages(res.pages || 1);
        } catch (_) {
          const all = getQuotes().filter(q => q.service === 'pcb');
          setTotal(all.length);
  ...
  const handleCreatePI = (quote) => {
    ...
    if (quote.proformaInvoice) {
      ...
      const items = existingItems.length > 0 ? existingItems : [buildDefaultItem()];

      setPiData({
        items,
        notes: existing.notes || '',
        terms: existing.terms || '100% advance payment required. Delivery timeline: 7–10 business days.',
        taxRate: typeof existing.taxRate === 'number' ? existing.taxRate : 0,
  ...
  const calculatePITotals = () => {
    const subtotal = piData.items.reduce((sum, item) => sum + item.totalPrice, 0);
    const discountAmount = subtotal * (piData.discountPercentage / 100);
    const taxableAmount = subtotal - discountAmount;
    const taxAmount = taxableAmount * (piData.taxRate / 100);
  ...
  const handleSendPI = async () => {
    try {
      ...
      await api.sendProformaInvoice(piFor._id || piFor.id, t);
  ...
  return (
    <AdminLayout admin={admin} onLogout={handleSignOut}>
      <Helmet>
        <title>Admin PCB Quotes | PCB Xpress</title>
        <meta name="robots" content="noindex,nofollow" />
      </Helmet>

      <div className="grid gap-8">
        <Section
          title={`PCB Quotes (${total})`}
          quotes={quotes}
          onView={setViewQuote}
          onEdit={setEditFor}
