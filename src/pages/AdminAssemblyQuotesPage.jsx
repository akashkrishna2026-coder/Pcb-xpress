...
const AdminAssemblyQuotesPage = () => {
  ...
  const assemblyQuotes = useMemo(() => quotes.filter(q => q.service === 'pcb_assembly'), [quotes]);

  const fetchQuotes = async () => {
    const t = getAdminToken();
    if (t) {
      try {
        const res = await api.listAllQuotesAdmin(t, { limit, page, service: 'pcb_assembly' });
  ...
  return (
    <AdminLayout admin={admin} onLogout={handleSignOut}>
      <Helmet>
        <title>Admin Assembly Quotes | PCB Xpress</title>
        <meta name="robots" content="noindex,nofollow" />
      </Helmet>

      <div className="grid gap-8">
        <Section
          title={`Assembly Quotes (${assemblyQuotes.length})`}
          quotes={assemblyQuotes}
          onView={setViewQuote}
        ...
        <div className="fixed inset-0 z-[110] bg-black/50 flex items-start justify-center p-4" onClick={() => setSendFor(null)}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-6 text-sm">
              ...
              {/* Specs */}
              {sendFor.service === 'pcb_assembly' && sendFor.specsAssembly && (
                <div>
                  <p className="font-semibold mb-2">Specifications:</p>
                  <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                    <li>Board size: {sendFor.specsAssembly.boardWidthMm} x {sendFor.specsAssembly.boardHeightMm} mm</li>
                    <li>Layers: {sendFor.specsAssembly.layers}</li>
                    <li>Assembly type: {(sendFor.specsAssembly.assemblyType || '').toUpperCase()}</li>
                    <li>Components: {sendFor.specsAssembly.componentCount}</li>
                    <li>Solder type: {sendFor.specsAssembly.solderType}</li>
                    <li>Quantity: {sendFor.specsAssembly.quantity}</li>
                  </ul>
                </div>
              )}
              ...
              <div className="border-t pt-4 text-xs text-muted-foreground">
                ...
                <p>Thank you for choosing PCB Xpress for your Assembly needs.</p>
              </div>
                ...
                <tr key={q._id || q.id} className="border-b last:border-0">
                  ...
                  <td className="py-3 pr-4">
                    {q.service === 'pcb_assembly' && q.specsAssembly ? (
                      <>
                        {(q.specsAssembly.assemblyType || '').toUpperCase()} · {q.specsAssembly.componentCount} components · {q.specsAssembly.boardWidthMm} x {q.specsAssembly.boardHeightMm}mm · Qty {q.specsAssembly.quantity}
                      </>
                    ) : (
  ...
  <div className="fixed inset-0 z-[100] bg-black/50 flex items-start justify-center p-4" onClick={onClose}>
    <div className="bg-background rounded-lg shadow-xl w-full max-w-3xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="p-6 grid sm:grid-cols-2 gap-4 text-sm">
        ...
        {quote.service === 'pcb_assembly' && quote.specsAssembly && (
          <>
            <Detail label="Board Size" value={`${quote.specsAssembly.boardWidthMm} x ${quote.specsAssembly.boardHeightMm} mm`} />
            <Detail label="Layers" value={String(quote.specsAssembly.layers)} />
            <Detail label="Assembly Type" value={(quote.specsAssembly.assemblyType || '').toUpperCase()} />
            <Detail label="Components" value={String(quote.specsAssembly.componentCount)} />
            <Detail label="Solder Type" value={quote.specsAssembly.solderType || '-'} />
            <Detail label="Quantity" value={String(quote.specsAssembly.quantity)} />
          </>
        )}
...
export default AdminAssemblyQuotesPage;
