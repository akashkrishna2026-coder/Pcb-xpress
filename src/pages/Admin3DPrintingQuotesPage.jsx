...
const Admin3DPrintingQuotesPage = () => {
  ...
  const printingQuotes = useMemo(() => quotes.filter(q => q.service === '3dprinting'), [quotes]);

  const fetchQuotes = async () => {
    const t = getAdminToken();
    if (t) {
      try {
        const res = await api.listAllQuotesAdmin(t, { limit, page, service: '3dprinting' });
  ...
  return (
    <AdminLayout admin={admin} onLogout={handleSignOut}>
      <Helmet>
        <title>Admin 3D Printing Quotes | PCB Xpress</title>
        <meta name="robots" content="noindex,nofollow" />
      </Helmet>

      <div className="grid gap-8">
        <Section
          title={`3D Printing Quotes (${printingQuotes.length})`}
          quotes={printingQuotes}
          onView={setViewQuote}
        ...
        <div className="fixed inset-0 z-[110] bg-black/50 flex items-start justify-center p-4" onClick={() => setSendFor(null)}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-6 text-sm">
              ...
              {/* Specs */}
              {sendFor.service === '3dprinting' && sendFor.specs3d && (
                <div>
                  <p className="font-semibold mb-2">Specifications:</p>
                  <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
                    <li>Process: {sendFor.specs3d.tech?.toUpperCase()}</li>
                    <li>Material: {sendFor.specs3d.material}</li>
                    <li>Resolution: {sendFor.specs3d.resolution}</li>
                    {sendFor.specs3d.tech === 'fdm' && (<li>Infill: {sendFor.specs3d.infillPercent}%</li>)}
                    <li>Size: {sendFor.specs3d.dims?.xMm}×{sendFor.specs3d.dims?.yMm}×{sendFor.specs3d.dims?.zMm} mm</li>
                    <li>Quantity: {sendFor.specs3d.quantity}</li>
                  </ul>
                </div>
              )}
              ...
              <div className="border-t pt-4 text-xs text-muted-foreground">
                ...
                <p>Thank you for choosing PCB Xpress for your 3D Printing needs.</p>
              </div>
...
const Section = ({ title, quotes, onView, onRemove, onSend, onViewProof, onPaymentStatusChange }) => (
  <Card>
      ...
      {quotes.length === 0 ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {quotes.map((q) => {
                const paymentStatus = q.paymentProof?.status || 'not_submitted';
                const paymentMethod = q.paymentProof?.proofFile ? 'bank_transfer' : (q.status === 'sent' ? 'cash' : 'pending');
                const shouldShowApprove = paymentStatus === 'approved' && paymentMethod === 'cash';
                console.log(`Quote ${q._id || q.id}: paymentStatus=${paymentStatus}, paymentMethod=${paymentMethod}, shouldShowApprove=${shouldShowApprove}`);
                return (
                  ...
                  <td className="py-3 pr-4">
                    {q.contact?.name || '(no name)'}<br />
                    <span className="text-xs text-muted-foreground">{q.contact?.email || '-'}</span>
                  </td>
                  <td className="py-3 pr-4">
                    {q.service === '3dprinting' && q.specs3d ? (
                      <>
                        {q.specs3d.tech?.toUpperCase()} {q.specs3d.dims?.xMm}×{q.specs3d.dims?.yMm}×{q.specs3d.dims?.zMm}mm · {q.specs3d.material} · Qty {q.specs3d.quantity}
                      </>
                    ) : (
                    ...
                    <div className="flex gap-2">
                      ...
                      {shouldShowApprove && (
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            // Navigate to AdminMfgApprovalPage with 3dprinting service
                            window.location.href = '/pcbXpress/admin/mfg-approval?service=3dprinting';
                          }}
                        >
                          Approve
                        </Button>
  ...
  <div className="fixed inset-0 z-[100] bg-black/50 flex items-start justify-center p-4" onClick={onClose}>
    <div className="bg-background rounded-lg shadow-xl w-full max-w-3xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="p-6 grid sm:grid-cols-2 gap-4 text-sm">
        ...
        {quote.service === '3dprinting' && quote.specs3d && (
          <>
            <Detail label="Process" value={quote.specs3d.tech?.toUpperCase()} />
            <Detail label="Material" value={quote.specs3d.material} />
            <Detail label="Resolution" value={quote.specs3d.resolution} />
            {quote.specs3d.tech === 'fdm' && (<Detail label="Infill" value={`${quote.specs3d.infillPercent}%`} />)}
            <Detail label="Size (X×Y×Z)" value={`${quote.specs3d.dims?.xMm}×${quote.specs3d.dims?.yMm}×${quote.specs3d.dims?.zMm} mm`} />
            <Detail label="Quantity" value={String(quote.specs3d.quantity)} />
          </>
        )}
...
export default Admin3DPrintingQuotesPage;
